  ...
  describe('Import policy from file', () => {
    test('Importing a syntactically valid policy from a file should succeed', () => {
      ...
      const policy = Policy.fromFile(
        ...
        'ImportedPolicy',
        {
          path: path.join(__dirname, 'test-policies', 'validPolicy1.cedar'),
          policyStore,
        },
      );
      ...
      expect(policyStatement).toEqual('permit(principal == PodcastApp::User::"abc", action, resource in PodcastApp::Application::"PodcastApp");');
    });
